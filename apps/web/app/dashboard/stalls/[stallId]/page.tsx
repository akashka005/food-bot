...
import { ArrowLeft, Store, Clock, Star, Flame, Utensils, Plus } from 'lucide-react';
import type { Metadata } from 'next';

export const metadata: Metadata = { title: 'Stall Menu' };

export default async function StallDetailPage({ params }: { params: Promise<{ stallId: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-12 pb-24">
      ...
      {/* Navigation */}
      <Link href="/dashboard/stalls" className="inline-flex items-center gap-2 font-black uppercase hover:text-primary transition-colors border-2 border-foreground bg-white px-4 py-2 rounded-xl brutal-shadow-sm brutal-shadow-hover">
          ...
          {stall.menuItems.length === 0 ? (
            ...
            stall.menuItems.map((item, i) => (
              <div key={item.id} className="group bg-white border-4 border-foreground rounded-2xl p-5 brutal-shadow brutal-shadow-hover flex flex-col transition-transform hover:-translate-y-1">
                ...
                <button className="w-full bg-foreground text-white font-black uppercase py-3 rounded-xl border-2 border-foreground flex items-center justify-center gap-2 hover:bg-primary brutal-shadow-sm transition-colors">
